...
def rollout(board: Board, state):
    ...
    # Get all legal actions
        ...
        avoiding_losses = [action for action in legal_actions 
                              if not board.is_ended(board.next_state(state, action))]

        # If there is only a "losing" action remaining
        if not avoiding_losses:
            avoiding_losses = legal_actions'''

        # Check for an immediate winning action
        for action in legal_actions:
            future_state = board.next_state(state, action)
            if board.is_ended(future_state):  # Winning or terminal state
                return future_state

        # Choose a random action from the remaining options
        '''if avoiding_losses:
            action = choice(avoiding_losses)
        elif legal_actions: 
